...
import type { RiskLevel, ScanOptions } from '../src/types.js';
...
async function postPrComment(token: string, body: string): Promise<void> {
  ...
  if (!GITHUB_REPOSITORY || !GITHUB_EVENT_PATH) return;

  const event = JSON.parse(readFileSync(GITHUB_EVENT_PATH, 'utf-8')) as {
...
async function run(): Promise<void> {
  const opts: ScanOptions = {
    ...
    format:      'markdown',
    failOn:      getInput('fail-on', 'high') as RiskLevel,
    concurrency: 8,
  ...
  try {
    ...
    if (threshold > 0) {
      const breached = result.results.some(
    ...
    console.error('ossrisk action failed:', (err as Error).message);
    process.exit(2);
  }
}
