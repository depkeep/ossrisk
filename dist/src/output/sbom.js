...
function declaredLicense(dep) {
    const sig = dep.signals.find((s) => s.type === 'license');
    // A resolved-but-unknown license is not something we can assert.
    if (!sig || sig.category === 'unknown')
        return undefined;
    return sig.license;
}
// ---- CycloneDX 1.5 (JSON) -------------------------------------------------
...
export function renderCycloneDx(result, toolVersion) {
...
// ---- SPDX 2.3 (JSON) ------------------------------------------------------
export function renderSpdx(result, toolVersion) {
