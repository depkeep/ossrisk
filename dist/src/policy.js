...
const runOpaProcess = (args, stdin, cwd) => new Promise(resolvePromise => {
    ...
    child.on('error', (err) => resolvePromise({ code: null, stdout, stderr, notFound: err.code === 'ENOENT' }));
...
export async function evaluatePolicy(result, policyPath, run = runOpaProcess) {
    ...
    if (value === undefined) {
        throw new Error(`policy at ${policyPath} does not define data.ossrisk.deny ` +
