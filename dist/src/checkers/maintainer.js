...
function chronologicalVersions(p) {
    const time = p.time ?? {};
...
function detectNewPublisher(p) {
    const age = ageDays(p);
    if (age === null || age < MIN_AGE_DAYS_FOR_NEW_PUBLISHER)
        return null;
    const ordered = chronologicalVersions(p);
    if (ordered.length < EARLY_RELEASE_WINDOW + 1)
        return null;
    ...
    if (!latestPublisher)
        return null;
    const earlyPublishers = new Set();
    for (const v of ordered.slice(0, EARLY_RELEASE_WINDOW)) {
        const name = versions[v]?._npmUser?.name;
        if (name)
            earlyPublishers.add(name);
    }
    if (earlyPublishers.size === 0)
        return null;
    if (earlyPublishers.has(latestPublisher))
        return null;
    return {
        type: 'maintainer',
        pattern: 'new-publisher',
        detail: `latest release published by "${latestPublisher}", ` +
            `who did not publish any of the first ${EARLY_RELEASE_WINDOW} versions`,
    };
}
function detectSoleMaintainer(p) {
...
export async function checkMaintainer(dep) {
