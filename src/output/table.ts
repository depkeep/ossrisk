...
function col(text: string, level: RiskLevel): string {
...
export function renderTable(result: ScanResult): string {
  ...
  lines.push(
    `  ${C.bold}${'Package'.padEnd(nameW)}${'Version'.padEnd(14)}${'Risk'.padEnd(11)}Details${C.reset}`
  );
  lines.push('  ' + '─'.repeat(84));

  for (const dep of result.results) {
    const details = dep.signals.map(s => {
      if (s.type === 'cve')       return `${s.id} (${s.severity})`;
      if (s.type === 'eol')       return `EOL ${s.eolDate}`;
