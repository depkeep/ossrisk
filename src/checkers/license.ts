import type { Dependency, LicenseCategory, LicenseSignal } from '../types.js';
...
const PYPI_CLASSIFIER_MAP: Record<string, string> = {
...
function normalize(raw: string): string {
  ...
  if (/^AGPL[\s\-v]?3/i.test(trimmed))               return 'AGPL-3.0';
  if (/^GPL[\s\-v]?3/i.test(trimmed))                return 'GPL-3.0';
  if (/^GPL[\s\-v]?2/i.test(trimmed))                return 'GPL-2.0';
  if (/^LGPL[\s\-v]?3/i.test(trimmed))               return 'LGPL-3.0';
  if (/^LGPL[\s\-v]?2/i.test(trimmed))               return 'LGPL-2.1';
  if (/^MPL[\s\-v]?2/i.test(trimmed))                return 'MPL-2.0';
  if (/^EPL[\s\-v]?2/i.test(trimmed))                return 'EPL-2.0';
  if (/^EPL[\s\-v]?1/i.test(trimmed))                return 'EPL-1.0';
  if (/^SSPL/i.test(trimmed))                        return 'SSPL-1.0';
...
async function pypiLicense(name: string): Promise<string | null> {
  try {
    const res = await fetch(`https://pypi.org/pypi/${encodeURIComponent(name)}/json`);
    if (!res.ok) return null;
    const data = await res.json() as PypiData;
...
export async function checkLicense(dep: Dependency): Promise<LicenseSignal[]> {
  try {
    let raw: string | null = null;

    if (dep.ecosystem === 'npm')  raw = await npmLicense(dep.name);
    if (dep.ecosystem === 'pypi') raw = await pypiLicense(dep.name);

    if (dep.ecosystem !== 'npm' && dep.ecosystem !== 'pypi') return [];
