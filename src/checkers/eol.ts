...
function candidateCycles(version: string): string[] {
  const parts = version.split('.').filter(p => /^\d+$/.test(p));
  const cycles: string[] = [];
  if (parts.length >= 2) cycles.push(`${parts[0]}.${parts[1]}`);
  if (parts.length >= 1) cycles.push(parts[0]);
  return cycles;
}

export async function checkEol(dep: Dependency): Promise<EolSignal[]> {
  ...
  try {
    ...
    for (const candidate of candidates) {
      ...
      const { eol } = cycle;
      if (eol === false) return [];                               // still supported
      if (eol === true)  return [{ type: 'eol', cycle: candidate, eolDate: 'unknown' }];
